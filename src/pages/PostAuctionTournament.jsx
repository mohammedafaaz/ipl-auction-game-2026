...
import { sortedTable, applyResultToTable, generatePlayoffs, simulateMatch, generateSchedule, buildPointsTable } from '../utils/tournament.js';
import TeamBadge from '../components/TeamBadge.jsx';

// MODE 2: Post-Auction Tournament (User's auction squad, simulate option, parallel matches)
export default function PostAuctionTournament() {
  ...
  const myTeamId = sessionStorage.getItem('soloTeamId');
  ...
  useEffect(() => {
    const loadTournament = () => {
      ...
      const existingTournament = sessionStorage.getItem('postAuctionTournamentData');
      
      if (existingTournament) {
        const d = JSON.parse(existingTournament);
        ...
        if (teamIds.length > 0) {
          ...
          sessionStorage.setItem('postAuctionTournamentData', JSON.stringify(newTournament));
        }
      }
    };
  ...
  const handlePlayMatch = useCallback((match) => {
    ...
    sessionStorage.setItem('postAuctionMode', 'true');
    sessionStorage.removeItem('playoffStage');
    navigate(`/hand-cricket/post-auction`);
  }, [navigate]);
  ...
  const handleSimulateMatch = useCallback((match) => {
    ...
    sessionStorage.setItem('postAuctionTournamentData', JSON.stringify(updatedTournament));
